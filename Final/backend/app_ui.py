...
with st.sidebar:
    ...
    import os
    default_backend = os.environ.get("BACKEND_API_URL", "http://localhost:8000")
    backend_url = st.text_input("Backend API Base URL", value=default_backend)
    
    st.markdown("---")
    st.markdown("### 🔐 Security & RBAC Identity")
    user_role = st.selectbox("Active Session Role", ["Admin", "Student", "Recruiter", "Guest"], index=0)
    user_email = f"{user_role.lower()}_dev@srm.edu.in"
    st.info(f"Identity: {user_email}")
...
# Prepare headers for the backend auth guards validation
req_headers = {
        ...
        with st.spinner(f"Extracting intelligence for '{company_name}'... (Running parallel researchers & self-healing validation)"):
            try:
                ...
                if res.status_code == 200:
                    ...
                    st.error("Access Denied: You do not have permission to trigger research pipelines.")
                else:
            ...
            if save_btn:
                with st.spinner("Writing payloads to Supabase..."):
                    try:
                        ...
                        elif save_res.status_code == 403:
                            st.session_state.save_status = {"success": False, "message": "Access Denied: Only Admins can save data profiles."}
                        else:
                            st.session_state.save_status = {"success": False, "message": f"Server error ({save_res.status_code}): {save_res.text}"}
