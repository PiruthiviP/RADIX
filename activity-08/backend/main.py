...
from auth_guards import verify_admin, verify_student_or_admin
...
@app.post("/api/search", dependencies=[Depends(verify_student_or_admin)])
...
@app.get("/api/similarity/{company_id}", dependencies=[Depends(verify_student_or_admin)])
...
@app.get("/api/clusters", dependencies=[Depends(verify_student_or_admin)])
def get_clusters():
    try:
...
@app.get("/api/ml-metadata", dependencies=[Depends(verify_student_or_admin)])
def get_ml_metadata():
    return {
...
@app.post("/api/predict", dependencies=[Depends(verify_student_or_admin)])
...
@app.post("/api/chatbot", dependencies=[Depends(verify_student_or_admin)])
