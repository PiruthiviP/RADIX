...
def test_startup_private_dataset_partial_profile_completeness():
    """
    Verifies that all startups and private companies in the generated 2.2.csv
    successfully pass mandatory field validation (success=True) despite having
    some optional parameters empty, demonstrating graceful degradation.
    """
    csv_path = os.path.join(os.path.dirname(__file__), "2.2.csv")
    dataset = load_csv_data(csv_path)
    
    assert len(dataset) > 0, f"Partial profiles dataset at {csv_path} is empty or missing."
    
    for row in dataset:
        company_name = row.get("name")
        success, richness_score, missing = calculate_profile_richness(row)
        
        # Verify mandatory validation is successful
        assert success is True, f"{company_name} failed validation. Missing mandatory: {missing}"
        
        # Verify it degrades gracefully (meaning at least some optional fields are missing)
        # 100% complete means all 163 populated; since it is a partial profile, it should be < 100%
        # unless it naturally had everything. Let's make sure it degrades gracefully for some:
        assert richness_score >= 15.34, f"{company_name} richness score is below base mandatory richness (~15.34%)."
...
def test_missing_mandatory_field_causes_validation_failure():
    ...
    # Mutate a row to remove a mandatory field (e.g. "name" / "Company Name")
...
def test_missing_optional_field_fails_only_score_richness():
    ...
    assert len(dataset) > 0
    # Take a row and clear an optional field (e.g. "short_name")
    mutated_row = dataset[0].copy()
...
if __name__ == "__main__":
    ...
    completed_csv_path = os.path.join(dir_path, "2.2.csv")
    
    master_out_csv = os.path.join(dir_path, "2.2_master_validation_results.csv")
    master_out_log = os.path.join(dir_path, "2.2_master_validation_results.log")
    
    completed_out_csv = os.path.join(dir_path, "2.2_completed_validation_results.csv")
    completed_out_log = os.path.join(dir_path, "2.2_completed_validation_results.log")
    ...
    print("\n" + "=" * 90)
    print("2. GENERATING VALIDATION REPORT FOR PARTIAL PROFILE STARTUP/PRIVATE DATASET")
    print("=" * 90)
    ...
    try:
        test_startup_private_dataset_partial_profile_completeness()
        print("✓ test_startup_private_dataset_partial_profile_completeness: PASSED")
