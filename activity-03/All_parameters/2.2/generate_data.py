...
# Mandatory columns from companies_master.csv
MANDATORY_COLUMNS = [
    'name', 'logo_url', 'category', 'incorporation_year', 'overview_text',
    'nature_of_company', 'headquarters_address', 'employee_size', 'pain_points_addressed',
    'focus_sectors', 'offerings_description', 'core_value_proposition', 'key_competitors',
    'website_url', 'social_media_followers', 'ceo_name', 'key_leaders', 'profitability_status',
    'sales_motion', 'remote_policy_details', 'strategic_priorities', 'go_to_market_strategy',
    'board_members', 'flexibility_level', 'company_maturity'
]
...
def generate_csv():
    ...
    output_path = "All_parameters/2.2/2.2.csv"
    ...
    for row in rows:
        ...
        # Select startups and private companies
        is_startup = 'startup' in cat
        is_private = 'private' in nature or 'private' in cat
        
        if is_startup or is_private:
            company_count += 1
            new_row = []
            for col_idx, val in enumerate(row):
                ...
                # If it's a mandatory column and empty, fill it
                if col_name in MANDATORY_COLUMNS and (val_clean == "" or val_clean.upper() == "NA" or val_clean.upper() == "NULL"):
    ...
    # Write the partial profile startup/private rows to the output CSV
    ...
    print(f"Successfully wrote {len(new_rows)} startup/private partial profiles to {output_path}.")
